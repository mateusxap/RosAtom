...
def generate_document(doc_num):
    try:
            ...
            if element == 'heading':
                ...
                if caption_style not in [s.name for s in document.styles]:
                    ...
                    try:
                        ...
                        caption_style_obj.font.size = Pt(12)  # На 2 размера меньше основного текста (14)
                ...
                if image_files:
                    ...
                    try:
                        ...
                        # Применяем стиль к подписи
                        for run in image_caption_paragraph.runs:
                            run.font.size = Pt(12)  # На 1-2 размера меньше основного текста
                ...
                if formula_files:
                    ...
                    try:
                        ...
                        # Применяем стиль к подписи
                        for run in formula_caption_paragraph.runs:
                            run.font.size = Pt(12)  # На 1-2 размера меньше основного текста
                            run.font.italic = True
                            run.font.name = 'Times New Roman'
                    except Exception as e:
        ...
        final_text = fake.text(max_nb_chars=random.randint(500, 1000))  # Изменено на 500-1000 символов
        final_paragraph = document.add_paragraph(final_text)
        final_paragraph.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
