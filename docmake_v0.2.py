...
def add_equation_to_docx(doc, equation_str, base_font_size=12, caption=True):
    """
    Добавляет формулу в документ с возможной подписью.

    :param doc: Объект документа Document.
    :param equation_str: Строка LaTeX-формулы.
    :param base_font_size: Базовый размер шрифта для подписи.
    :param caption: Добавлять ли подпись.
    """
    ...
    try:
        if caption:
            ...
            # Добавляем подпись к формуле
            caption_type = random.choice(["Формула", "Формула №"])
...
for doc_num in range(num_documents):
    ...
    for section_num in range(num_sections):
        ...
        if random.choice([True, False]):
            ...
            add_equation_to_docx(document, equation_str, base_font_size=base_font_size, caption=True)
