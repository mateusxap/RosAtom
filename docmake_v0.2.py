...
# Создаем папки для сохранения документов и изображений
...
for doc_num in range(num_documents):
    ...
    for section_num in range(num_sections):
        ...
        # Определяем количество контент-блоков в разделе
        num_content_items = random.randint(5, 10)

        for _ in range(num_content_items):
            possible_content_types = ['text', 'table', 'image', 'list_number', 'list_bullet', 'formula']
            ctype = random.choice(possible_content_types)

            # Проверяем условие для многоколоночного раздела
            if use_columns and ctype == 'text' and last_content_type == 'text':
                non_text_content_types = ['table', 'image', 'list_number', 'list_bullet', 'formula']
                ctype = random.choice(non_text_content_types)
            ...
            if ctype == 'text':
                ...
                # Устанавливаем 'keep_together' для предотвращения разбиения параграфа между колонками
                if use_columns:
                    paragraph.paragraph_format.keep_together = True
                ...
                # Добавляем разрыв колонки после текстового параграфа
                if use_columns:
                ...
                for _ in range(random.randint(3, 7)):
                    ...
                    # Устанавливаем 'keep_together' для элементов списка в многоколоночном разделе
                    if use_columns:
                        paragraph.paragraph_format.keep_together = True
                ...
                last_content_type = 'list_number'

            elif ctype == 'list_bullet':
                ...
                last_content_type = 'list_bullet'

            elif ctype == 'formula':
                equation_str = random.choice(FORMULAS)
                add_equation_to_docx(document, equation_str, caption=True)
                last_content_type = 'formula'

            else:
                # Если тип контента не распознан, пропускаем итерацию
                continue
